...
const conferenceConfigs: Record<ConferenceKey, ConferenceConfig> = {
  leyte: {
    ...
    bannerImage: `https://oilbgqmzxltdrksiypfi.supabase.co/storage/v1/object/public/Logo/assets/LEYTE_Empowered_With_Purpose.png?v=${imageVersion}`,
    registrationTitle:
  ...
  cebu: {
    ...
    bannerImage: `https://oilbgqmzxltdrksiypfi.supabase.co/storage/v1/object/public/Logo/assets/CEBU_Empowered_With_Purpose.png?v=${imageVersion}`,
    registrationTitle:
      "REGISTRATION FORM: CEBU CHRISTIAN LEADERSHIP CONFERENCE 2026",
    totalSlots: 100,
...
export default function Page() {
  const [selectedConference, setSelectedConference] =
    useState<ConferenceKey>("leyte");
  const [pendingConference, setPendingConference] =
    useState<ConferenceKey | null>(null);
  const [isSwitchingConference, setIsSwitchingConference] = useState(false);
  const [availableSlotsByConference, setAvailableSlotsByConference] = useState<
    Record<ConferenceKey, number>
  >({
  ...
  const conferenceQuery = useMemo(
    () => `conference=${conference.key}`,
    [conference.key],
  );
  const isRegistrationClosed =
    (availableSlotsByConference[conference.key] ?? conference.totalSlots) <= 0;
          ...
          >
            <div className="flex h-full flex-col gap-4 p-5 text-amber-50 sm:gap-3.5 sm:p-6">
              ...
              <div className="rounded-xl border border-amber-200/25 bg-slate-950/20 p-3 text-[0.8rem] leading-relaxed text-amber-100 sm:text-[0.92rem]">
                <p className="mb-2 mt-0 font-semibold">
                  For More details and Inquiries. Kindly contact the details
                  below:
                </p>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
                  <div className="space-y-1.5 rounded-lg border border-amber-200/20 bg-black/10 p-2">
                    <p className="m-0 font-medium text-amber-200">
                      Email Address
                    </p>
                    <a
                ...
                {isRegistrationClosed ? (
                  <div className="rounded-xl border border-rose-200/35 bg-[linear-gradient(135deg,rgba(244,63,94,0.16),rgba(217,119,6,0.14),rgba(30,41,59,0.65))] p-4 text-center shadow-[0_10px_18px_rgba(0,0,0,0.28)]">
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-amber-200">
                      Thank you for your overwhelming love and support. We are
                      deeply grateful for everyone who registered, and we pray
                      this conference will be a blessing to all attendees.
                    </p>
                  </div>
                ) : (
